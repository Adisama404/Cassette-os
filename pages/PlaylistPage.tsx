...
import { Play, Trash2, ArrowLeft, Plus, X, Shuffle, GripVertical } from 'lucide-react';
...
export const PlaylistPage: React.FC = () => {
    ...
    const handlePlay = (index: number = 0) => {
        loadPlaylist(playlist, index, true);
        navigate('/deck');
    };
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div className="flex mt-8 space-x-4 animate-enter stagger-2">
                        <button
                            ...
                            className="flex items-center space-x-2 bg-stone-800 text-green-500 px-6 py-3 rounded border border-stone-700 hover:bg-stone-700 hover:scale-105 transition-all shadow-[0_4px_10px_rgba(0,0,0,0.3)] active:translate-y-[2px] active:shadow-none active:scale-100"
                        >
                    ...
                    <div className="p-6 border-b border-stone-800 flex justify-between items-center bg-stone-900/80">
                        ...
                        <div className="relative flex items-center gap-2">
                            <button
                                ...
                                className={`flex items-center gap-2 px-3 py-1 font-mono text-xs border transition-colors rounded-sm ${isShuffled ? 'bg-green-900/40 text-green-400 border-green-500' : 'bg-stone-800 text-stone-500 border-stone-700 hover:text-stone-300'}`}
                                ...
                                return (
                                    <li
                                        ...
                                        onDrop={(e) => {
                                            ...
                                            if (fromIndex !== idx) {
                                                reorderTrack(playlist.id, fromIndex, idx);
                                            }
                                        }}
                                        className={`
                                    group flex items-center justify-between p-3 rounded font-mono text-xs cursor-pointer transition-all duration-300
                                    animate-enter hover:bg-stone-800
                                    ${isPlayingThis ? 'bg-green-900/20 text-green-400' : 'text-stone-500 hover:text-stone-300'}
                                `}
                                        onClick={() => isCurrentTape ? playTrack(idx) : handlePlay(idx)}
                                        style={{ animationDelay: `${idx * 50 + 200}ms` }}
                                    >
                                        <div className="flex items-center overflow-hidden flex-1 mr-4">
                                            <div
                                                className="mr-3 cursor-grab active:cursor-grabbing text-stone-700 hover:text-stone-500"
