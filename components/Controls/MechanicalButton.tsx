...
export const MechanicalButton: React.FC<MechanicalButtonProps> = ({
    ...
    let faceColor = 'bg-[var(--button-face)]';
    let sideColor = 'bg-[var(--button-side)]';
    ...
    const borderColor = color === 'red' ? 'border-red-950' : (color === 'orange' ? 'border-orange-950' : 'border-[var(--button-border)]');
    ...
    return (
        <div className={`flex flex-col items-center gap-3 group ${className} ${disabled ? 'opacity-40 pointer-events-none' : ''}`}>
            ...
            <button
                ...
                disabled={disabled}
                className={`
                relative w-14 h-11 sm:w-20 sm:h-14 rounded-md 
